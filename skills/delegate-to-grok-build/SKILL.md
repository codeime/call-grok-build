...
description: 调用本机已安装的 Grok Build CLI，处理边界明确的研究、计划、实现或代码审查任务，再由 Codex 独立核验结果。用于明确的 Grok Build 调用或跨模型复核；不要用于密钥或未经授权的外部变更。
---
...
- 一次实现修复流程最多安排一次针对修复结果的 Grok 回归复审，再安排一次 Luna Max 独立终审；不允许递归调用本插件、自动重试或自动重新委派。
...
- 一个 job 只创建一个全新的 ACP 进程、会话和 prompt。每个 ACP task 都显式禁用 `run_terminal_cmd,Agent`；Grok worker 不得运行 shell/interpreter/Git/测试，不得调用本插件、另一个 Grok worker 或 Codex 委托。
